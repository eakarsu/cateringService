...
function createCommerceApp({config,pool,providers}) {
  ...
  app.get('/api/auth/demo-credentials',(_req,res)=>{if(process.env.NODE_ENV==='production')return res.status(404).json({error:'Not found'});const email=process.env.PROVISION_ADMIN_EMAIL||process.env.ADMIN_EMAIL||'',password=process.env.PROVISION_ADMIN_PASSWORD||process.env.ADMIN_PASSWORD||'';if(!email||!password)return res.status(503).json({error:'Demo credentials unavailable'});res.set('cache-control','no-store');return res.json({email,password});});
